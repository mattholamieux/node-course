...
window.onload = (event) => {
    fetch('http://localhost:3000/locations').then((response) => {
        response.json().then((data) => {
            ...
            fetch('http://localhost:3000/locations?country=' + country).then((response) => {
                response.json().then((data) => {
                    const states = data;
...
stateSelect.addEventListener('change', () => {
    ...
    const country = countrySelect.value;
    fetch('http://localhost:3000/locations?state='+state+'&country='+country).then((response) => {
        response.json().then((data) => {
            const cities = data;
...
forecastForm.addEventListener('submit', (e) => {
    ...
    fetch('http://localhost:3000/forecast?city='+city+'&state='+state+'&country='+country).then((response) => {
        response.json().then((data) => {
            if (data.error){
